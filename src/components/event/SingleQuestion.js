...
export default function SingleQuestion({ event, question }) {
	...
	const [offline, setOffline] = React.useState(question.offline);

	const toggleAnswerStatus = async (newstatus) => {
		setAnswered(newstatus);
		let answered_value = "no";
		if (newstatus) {
			answered_value = "yes";
		}
		const endpoint = "/api/event/" + event.code;
		try {
	...
	const toggleOfflineStatus = async () => {
		...
		try {
			...
			if (response_content) {
				switch (response_content.outcome) {
					case 3:
						//console.log("Question taken offline");
						//location.reload();
						break;
					default:
						alert(response_content.outcome);
		...
		<>
			{question ? (
				<ListItem
					...
					sx={{
						backgroundColor: answered ? "#DCDCDC" : "white",
					}}
				>
					<ListItemAvatar>
						<Avatar alt="Annonymous" />
					</ListItemAvatar>
					<ListItemText
						...
						secondary={
							<React.Fragment>
								...
								<Button variant="text" color="primary" onClick={toggleUpVote}>
									<svg
